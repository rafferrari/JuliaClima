...
begin
    using DifferentialEquations, Plots, PlutoUI, LinearAlgebra, CSV, DataFrames, Images, MAT, XLSX
end
...
md"""
### Load historical sea level data for Venice
---
...
begin
	...
	file_mat = "/Users/raffaele/Work/Seminars/Venezia/2023/nhess-2020-351_data/VENICE_RSL_DATA.mat";

	# Load the contents of the .mat file
	mat_contents = matread(file_mat);

	# Access the variables stored in the .mat file
	data = mat_contents["VENICE_DATA"];
	year=data[1]["year"];
	rsl=data[1]["RSL"];
	rsl2=data[1]["VLMcorrectedRSL"];
	season=data[1]["season"];

end

# ╔═╡ 85b0d551-5818-4125-8e59-d0c5f4bcdeb8
begin
	plot(year, rsl,       legend = false, 
		 background_color_inside = :white,
                          ...
                          c      = :black,
                          lw     = :3)
	plot!(year, rsl2,       legend = false, 
		 background_color_inside = :white,
                          c      = :red,
                          lw     = :3)
	title!("Venice relative sea level rise")
end

# ╔═╡ e24adbc0-a183-4784-a8f1-1453567530d7
md"""
### Load sea level projections for Venice
---

"""

# ╔═╡ 2ddc1ab7-9be9-437b-a763-6f7375c943ce
begin
	
	# Specify the path to the .mat file
	file_xlsx = "/Users/raffaele/Work/Seminars/Venezia/2023/nhess-2020-351_data/data_msl_prj_share.xlsx"

	# Load the contents of the .mat file
	datacmip = XLSX.readxlsx(file_xlsx);

	# Get the names of all the sheets in the XLSX file
	cmip = datacmip["Foglio1"];

	# Access the variables stored in the .mat file
	yearc = cmip["A2:A95"][:,1];
	rcp6_05 = cmip["B2:B95"][:,1];
	rcp6_50 = cmip["C2:C95"][:,1];
	rcp6_95 = cmip["D2:D95"][:,1];
	rcp8_05 = cmip["F2:F95"][:,1];
	rcp8_50 = cmip["G2:G95"][:,1];
	rcp8_95 = cmip["H2:H95"][:,1];
	high = cmip["J2:J95"][:,1];

end

# ╔═╡ a5f9ca17-bd02-4cdc-8307-729dcb40f864
yearc[:,1]

# ╔═╡ 31afe23c-a5fe-44f5-92ca-1513b407b41c
begin
	plot(yearc, rcp6_05,       legend = false, 
		 background_color_inside = :white,
		                  xlabel = "Years",
	                      ylabel = "Sea level [m]",
                          #ylim   = (-273.15,400),
	...
	plot!(yearc,rcp8_50)
	plot!(yearc,rcp8_95)
	title!("Venice projected sea level rise")
end

# ╔═╡ 4af50b2b-2f39-4b6c-804a-72cca0ebc58e
md"""
...
# ╔═╡ 86a14ad7-7dd8-462e-a3e4-bb98020f6fb9
load("SLRVenice.jpg")

# ╔═╡ 0fe9ed48-44db-4952-aa46-72a3578c7b78
md"""

Long-term projections show great uncertainty in the relative sea level of Venice: it is predicted that by 2100 the sea level will rise between 17 and 120 cm. Davide Zanchettin, lead author of the article and professor at Ca’ Foscari University of Venice, attributes this uncertainty to the fact that projections are based on a wide range of emission scenarios, as well as on an inadequate understanding of the processes, both happening remotely and inside the Mediterranean and Adriatic sea, that contribute to changing sea level.   
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8bec3d86ab8897d5583a17a9d6717084a17ad0ee"
